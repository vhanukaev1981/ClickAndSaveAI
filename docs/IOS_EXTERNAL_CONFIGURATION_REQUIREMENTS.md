...
| **App Bundle Identifier** | iOS Lead | Apple Developer Portal | `com.aistudio.clickandsaveai.staging` (or `.app.staging`) | `com.aistudio.clickandsaveai.app` | Defined in project build configurations. |
...
| **App Check Provider (Production)** | DevOps | Firebase Console -> App Check -> Apps | DeviceCheck / App Attest configured with Apple Team ID | DeviceCheck / App Attest configured with Apple Team ID & Private Key | Enforced at 100% on all backend callables. |
...
- **Bundle ID**: `com.aistudio.clickandsaveai.staging`
...
- **App Check Mode**:
  ...
  - TestFlight: `AppAttestProviderFactory` (or `DeviceCheckProviderFactory` fallback).
- **CI Secret Names**:
...
- **Bundle ID**: `com.aistudio.clickandsaveai.app`
- **Google Web Client ID**: Loaded dynamically or configured from production GCP credentials.
- **App Check Mode**:
  - Strictly `AppAttestProviderFactory` with `DeviceCheckProviderFactory` fallback.
