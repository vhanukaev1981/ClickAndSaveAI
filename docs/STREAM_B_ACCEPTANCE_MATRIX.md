...
| Progress | Customer state language is backend-driven and limited to truthful detected/checked/verified/still-checking semantics | `TruthfulProgressPresentationPolicy` | `TruthfulProgressPresentationPolicyTest` |
| Motion | Motion communicates real work only; no fake percentages or timer-driven stage advancement | `docs/STREAM_B_MOTION_CONTRACT.md` | `StreamBMotionContractTest` |
...
| Bills / payment handoff | Pay CTA may exist only for a Core-verified official provider payment destination; Click&SaveAI does not process/store cards | Core trusted payment-handoff candidate required | `STREAM_B_P0_GAP_MAP.md`, device E2E evidence |
...
| Provider handoff | UI communicates recommend/connect/refer; it never claims Click&SaveAI performs switching or activation without evidence | exact-offer consent/handoff contract | `StreamBConsentPrivacyContractTest`, `STREAM_B_P0_GAP_MAP.md` |
...
| Accessibility / motion | Reduced motion never removes status information; animation is never the only signal | `docs/STREAM_B_MOTION_CONTRACT.md` | `StreamBMotionContractTest` |
...
| Customer copy | No Firebase/App Check/backend/CRM/lead/commission/attribution language | all customer screens | `CustomerVisibleCopyGuardTest`, `CustomerUiSourceGuardTest`, `CustomerAuthCopySanitizationTest` |
| Consent | App never claims an automatic provider switch | Savings/Profile/Settings copy | `StreamBConsentPrivacyContractTest` |
| Evidence | Device acceptance preserves exact CI→APK→device SHA chain and captures failures | `docs/STREAM_B_DEVICE_EVIDENCE_TEMPLATE.md` | `StreamBDeviceEvidenceContractTest`, `StreamBReleaseReadinessContractTest` |

## Integration acceptance

Stream B is integration-ready only after:

1. Stream A promotes a validated staging baseline and completes the required device E2E correction cycle.
...
6. `docs/STREAM_B_DEVICE_EVIDENCE_TEMPLATE.md` records the exact same CI/APK/device SHA chain.
7. No customer-facing copy exposes implementation or commercial internals and no UI fabricates savings/economic/progress data.
8. Live progress/motion is enabled only for explicit evidence-backed Core states; no timer/percentage simulation.
9. No payment CTA is presented as verified provider payment unless Core supplies a trusted verified payment destination; Click&SaveAI never stores/processes card details in this phase.
10. Provider handoff never implies Click&SaveAI executed a switch, activation or conversion without corresponding evidence.
